...
def make_color_wheel():
    ...
    col = 0
    # RY
    colorwheel[0:RY, 0] = 4096
    colorwheel[0:RY, 1] = np.transpose(np.floor(4096*np.arange(0, RY) / RY))
    col += RY
    # YG
    colorwheel[col:col+YG, 0] = 4096 - np.transpose(np.floor(4096*np.arange(0, YG) / YG))
    colorwheel[col:col+YG, 1] = 4096
    col += YG
    # GC
    colorwheel[col:col+GC, 1] = 4096
    colorwheel[col:col+GC, 2] = np.transpose(np.floor(4096*np.arange(0, GC) / GC))
    col += GC
    # CB
    colorwheel[col:col+CB, 1] = 4096 - np.transpose(np.floor(4096*np.arange(0, CB) / CB))
    colorwheel[col:col+CB, 2] = 4096
    col += CB
    # BM
    colorwheel[col:col+BM, 2] = 4096
    colorwheel[col:col+BM, 0] = np.transpose(np.floor(4096*np.arange(0, BM) / BM))
    col += + BM
    # MR
    colorwheel[col:col+MR, 2] = 4096 - np.transpose(np.floor(4096 * np.arange(0, MR) / MR))
    colorwheel[col:col+MR, 0] = 4096
    return colorwheel
...
def compute_color(u,v):
    ...
    for i in range(np.size(colorwheel,1)):
        ...
        col[notidx] *= 0.75
        img[:, :, i] = np.uint8(np.floor(4096 * col*(1-nanIdx)))
    return img
